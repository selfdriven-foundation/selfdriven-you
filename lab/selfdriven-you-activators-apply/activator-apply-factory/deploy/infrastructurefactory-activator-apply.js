...
module.exports =
{
    ...
    init: function (param)
    {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-get-deploy-settings',
            code: function ()
            {
                const settings = entityos.get({ scope: '_settings' });
                ...
                return {
                    ...
                    applicationsBucket: _.get(settings, 'deploy.applicationsBucket', 'selfdriven-activator-applications'),
                    ...
                    lambdaZipName:      _.get(settings, 'deploy.lambdaZipName', 'activator-apply-lambda.zip'),
                    functionName:       _.get(settings, 'deploy.functionName', 'activator-apply'),
                    roleName:           _.get(settings, 'deploy.roleName', 'activator-apply-lambda-role'),
        ...
        entityos.add(
        {
            name: 'app-process-aws-activator-apply-deploy',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');

                console.log('\n── activator-apply deploy ─────────────');
                ...
                entityos.invoke('util-aws-activator-apply-deploy-s3-bucket-check');
            }
        });
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-s3-bucket-check',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                ...
                .then(function ()
                {
                    ...
                    entityos.invoke('util-aws-activator-apply-deploy-applications-bucket-check');
                })
                .catch(function (err)
                {
                    if (err.$metadata?.httpStatusCode === 404 || err.name === 'NotFound' || err.name === 'NoSuchBucket')
                    {
                        entityos.invoke('util-aws-activator-apply-deploy-s3-bucket-create');
                    }
                    else
                    {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-s3-bucket-create',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-applications-bucket-check',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const { S3Client, HeadBucketCommand } = require('@aws-sdk/client-s3');
                ...
                .then(function ()
                {
                    ...
                    entityos.invoke('util-aws-activator-apply-deploy-sns-topic');
                })
                .catch(function (err)
                {
                    if (err.$metadata?.httpStatusCode === 404 || err.name === 'NotFound' || err.name === 'NoSuchBucket')
                    {
                        entityos.invoke('util-aws-activator-apply-deploy-applications-bucket-create');
                    }
                    else
                    {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-applications-bucket-create',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-sns-topic',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-zip',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                ...
                output.on('close', function ()
                {
                    ...
                    entityos.set({ scope: 'activator-apply-deploy', context: 'zipPath', value: zipPath });
                    entityos.invoke('util-aws-activator-apply-deploy-s3-upload');
                });
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-s3-upload',
            code: function ()
            {
                const deploy  = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const zipPath = entityos.get({ scope: 'activator-apply-deploy', context: 'zipPath' });
                ...
                .then(function (response)
                {
                    ...
                    entityos.set({ scope: 'activator-apply-deploy', context: 's3Key', value: s3Key });
                    entityos.invoke('util-aws-activator-apply-deploy-iam-role-check');
                })
                .catch(function (err)
                {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-iam-role-check',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const { IAMClient, GetRoleCommand } = require('@aws-sdk/client-iam');
                const iam = new IAMClient(entityos.invoke('util-aws-activator-apply-get-config'));
                ...
                .then(function (response)
                {
                    ...
                    entityos.set({ scope: 'activator-apply-deploy', context: 'roleARN', value: response.Role.Arn });
                    // Ensure the inline policy reflects current bucket/topic on re-runs.
                    entityos.invoke('util-aws-activator-apply-deploy-iam-role-policy');
                })
                .catch(function (err)
                {
                    if (err.name === 'NoSuchEntityException')
                    {
                        entityos.invoke('util-aws-activator-apply-deploy-iam-role-create');
                    }
                    else
                    {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-iam-role-create',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-iam-role-policy',
            code: function ()
            {
                const deploy   = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const topicArn = entityos.get({ scope: 'activator-apply-deploy', context: 'snsTopicArn' });
                ...
                iam.send(new PutRolePolicyCommand({
                    RoleName: deploy.roleName,
                    PolicyName: 'activator-apply-write-and-notify',
                    PolicyDocument: JSON.stringify(policy)
                }))
                .then(function ()
                {
                    console.log('  ✓ Inline policy applied — waiting 10s for IAM propagation');
                    setTimeout(function () { entityos.invoke('util-aws-activator-apply-deploy-lambda-check'); }, 10000);
                })
                .catch(function (err)
                {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-lambda-check',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                ...
                .then(function (response)
                {
                    ...
                    entityos.set({ scope: 'activator-apply-deploy', context: 'functionARN', value: response.Configuration.FunctionArn });
                    entityos.invoke('util-aws-activator-apply-deploy-lambda-update');
                })
                .catch(function (err)
                {
                    if (err.name === 'ResourceNotFoundException')
                    {
                        entityos.invoke('util-aws-activator-apply-deploy-lambda-create');
                    }
                    else
                    {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-lambda-create',
            code: function ()
            {
                const deploy   = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const roleARN  = entityos.get({ scope: 'activator-apply-deploy', context: 'roleARN' });
                const s3Key    = entityos.get({ scope: 'activator-apply-deploy', context: 's3Key' });
                const topicArn = entityos.get({ scope: 'activator-apply-deploy', context: 'snsTopicArn' });
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-lambda-update',
            code: function ()
            {
                const deploy   = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const roleARN  = entityos.get({ scope: 'activator-apply-deploy', context: 'roleARN' });
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-apigw-check',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const { APIGatewayClient, GetRestApisCommand } = require('@aws-sdk/client-api-gateway');
                const apigw = new APIGatewayClient(entityos.invoke('util-aws-activator-apply-get-config'));
                ...
                .then(function (response)
                {
                    ...
                    if (existing)
                    {
                        ...
                        entityos.set({ scope: 'activator-apply-deploy', context: 'apiId', value: existing.id });
                        entityos.invoke('util-aws-activator-apply-deploy-apigw-configure');
                    }
                    else
                    {
                        entityos.invoke('util-aws-activator-apply-deploy-apigw-create');
                    }
                })
                .catch(function (err)
                {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-apigw-create',
            code: function ()
            {
                const deploy = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                ...
                apigw.send(new CreateRestApiCommand({
                    name: deploy.apiName,
                    description: 'selfdriven.you activator application intake',
                    endpointConfiguration: { types: ['REGIONAL'] }
                }))
                .then(function (response)
                {
                    console.log('  ✓ REST API created:', response.id);
                    entityos.set({ scope: 'activator-apply-deploy', context: 'apiId', value: response.id });
                    entityos.invoke('util-aws-activator-apply-deploy-apigw-configure');
                })
                .catch(function (err)
                {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-apigw-configure',
            code: function ()
            {
                const deploy      = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const apiId       = entityos.get({ scope: 'activator-apply-deploy', context: 'apiId' });
                const functionARN = entityos.get({ scope: 'activator-apply-deploy', context: 'functionARN' });
                ...
                const cfg = entityos.invoke('util-aws-activator-apply-get-config');
                ...
                .then(function ()
                {
                    ...
                    return lambda.send(new AddPermissionCommand({
                        FunctionName: deploy.functionName,
                        StatementId: 'apigw-invoke-activator-apply',
                ...
                .then(function ()
                {
                    ...
                    return apigw.send(new CreateDeploymentCommand({
                        ...
                        description: 'activator-apply deploy ' + new Date().toISOString()
                    }));
                })
                .then(function ()
                {
                    ...
                    entityos.set({ scope: 'activator-apply-deploy', context: 'invokeUrl', value: invokeUrl });
                    entityos.set({ scope: 'activator-apply-deploy', context: 'stageArn',
                        value: 'arn:aws:apigateway:' + deploy.region + '::/restapis/' + apiId + '/stages/' + deploy.apiStage });
                    console.log('  ✓ Deployed:', invokeUrl);
                    entityos.invoke('util-aws-activator-apply-deploy-waf');
                })
                .catch(function (err)
                {
        ...
        entityos.add(
        {
            name: 'util-aws-activator-apply-deploy-waf',
            code: function ()
            {
                const deploy   = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const stageArn = entityos.get({ scope: 'activator-apply-deploy', context: 'stageArn' });

                const {
                    WAFV2Client, ListWebACLsCommand, CreateWebACLCommand, AssociateWebACLCommand
                } = require('@aws-sdk/client-wafv2');
                const waf = new WAFV2Client(entityos.invoke('util-aws-activator-apply-get-config'));
                ...
                const rules = [{
                    ...
                    VisibilityConfig: {
                        ...
                        MetricName: 'activatorApplyRateLimit'
                    }
                }];
                ...
                const associate = function (webAclArn)
                {
                    ...
                    .then(function ()
                    {
                        ...
                        entityos.set({ scope: 'activator-apply-deploy', context: 'webAclArn', value: webAclArn });
                        entityos.invoke('app-process-aws-activator-apply-deploy-done');
                    })
                    .catch(function (err)
                    {
                    ...
                    else
                    {
                        waf.send(new CreateWebACLCommand({
                            ...
                            VisibilityConfig: {
                                SampledRequestsEnabled: true,
                                CloudWatchMetricsEnabled: true,
                                MetricName: 'activatorApplyWebAcl'
                            },
                            Description: 'Rate limiting for selfdriven.you activator intake'
                        }))
        ...
        entityos.add(
        {
            name: 'app-process-aws-activator-apply-deploy-done',
            code: function ()
            {
                const deploy      = entityos.invoke('util-aws-activator-apply-get-deploy-settings');
                const functionARN = entityos.get({ scope: 'activator-apply-deploy', context: 'functionARN' });
                const invokeUrl   = entityos.get({ scope: 'activator-apply-deploy', context: 'invokeUrl' });
                const topicArn    = entityos.get({ scope: 'activator-apply-deploy', context: 'snsTopicArn' });
                const webAclArn   = entityos.get({ scope: 'activator-apply-deploy', context: 'webAclArn' });
